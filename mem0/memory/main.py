        ...
        for _attempt in range(3):
            try:
                ...
                try:
                    ...
                    # Try extracting JSON from cleaned response (handles chatty LLM output)
                    extracted_json = extract_json(cleaned_response)
        ...
        if new_retrieved_facts:
            ...
            try:
                if not response or not response.strip():
                    ...
                    try:
                        new_memories_with_actions = json.loads(cleaned, strict=False)
                    except json.JSONDecodeError:
                        extracted_json = extract_json(cleaned)
